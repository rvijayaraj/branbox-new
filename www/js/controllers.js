...
.controller('aboutUs', function($scope,$http) {
  ...
  db.transaction(function(tx){
    tx.executeSql('SELECT * FROM  businessDetails',[], function (tx, results){
        ...
        $scope.addressDtailsBrandName=results.rows.item(0).brandName;
        $scope.addressDtailsPhoneNumber1=results.rows.item(0).phoneNumber1;
        $scope.addressDtailsEmail1=results.rows.item(0).email1;
        $scope.addressDtailsAddress1=results.rows.item(0).address1;
        $scope.addressDtailsAddress2=results.rows.item(0).address2;
        $scope.addressDtailsCity=results.rows.item(0).city;
        $scope.addressDtailsState=results.rows.item(0).state;
        $scope.addressDtailsCountry=results.rows.item(0).country;
        //alert($scope.addressDtailsCountry);
       // alert(results.rows.item(0).country);
    });
  });
  db.transaction(function(tx){
...
.controller('MenuController', function($scope,$http,$location,alertmsg,$ionicPopup,$cordovaNetwork) {
    //alert('test');
                  ...
                  db.transaction(function(tx){
                    //alert("colorSetting start ");
                      tx.executeSql('SELECT * FROM colorSetting',[], function (tx, results)
                      {
                          ...
                          //alert(menudatas.item(0).headerColor);
                          //alert(menudatas.item(0).headerColor+"Header Color"+menudatas.item(0).currencyFormat+"Format");
...
.controller('SubMenuItemController', function($scope,$http,$location,alertmsg,$cordovaNetwork) {
    $("#sidebar").removeClass("toggled");
    $("#menu-trigger").removeClass("open");
    ...
    var json_arr=[];
    //alert(menuId+" and "+subMenuId);
    
    
        
                //var db = window.openDatabase("branboxnew", "1.0", "branbox Demo", 100 * 1024 * 1024);
                //db.transaction(function(tx){
                //    tx.executeSql('SELECT * FROM item where menuId="'+menuId+'" and subMenuId="'+subMenuId+'" ',[], function (tx, results)
                //    {
                //        var itemLength = results.rows.length;
                //        var menudatas=results.rows;
                //        //alert(itemLength+" inner Item");
                //        for(var i = 0; i < itemLength; i++)
                //        {
                //            var row=menudatas.item(i);
                //            //var obj = {id:row.id,businessId: row.businessId,menuId:row.menuId,subMenuId:row.subMenuId,name:row.name,image:row.image,price:row.price,tax:row.categtaxory,offers:row.offers};
                //            json_arr.push(row);
                //            alert(row.name);
                //        }
                //        
                    ...
                    tx.executeSql('SELECT * FROM item where menuId="'+menuId+'" and subMenuId="'+subMenuId+'" ',[], function (tx, results)
                    {
                       ...
                       // alert(itemLength+" inner Item");
                        for(var i = 0; i < itemLength; i++)
                        {
                            ...
                            //$scope.SubMenuItemData.push(menudatas.item(i));
                            var row=menudatas.item(i);
                            //var obj = {id:row.id,businessId: row.businessId,menuId:row.menuId,subMenuId:row.subMenuId,name:row.name,image:row.image,price:row.price,tax:row.categtaxory,offers:row.offers};
                            json_arr.push(row);
                            
                            tx.executeSql('SELECT * FROM itemings where menuId="'+menuId+'" and subMenuId="'+subMenuId+'" and itemId="'+menudatas.item(i).id+'" ',[], function (tx, resultsIng)
                            {
                                var itemLengthIng = resultsIng.rows.length;
                                //alert(itemLengthIng+" inner Ing");
                                ...
                                for(var j = 0; j < itemLengthIng; j++)
                                {
                                    ...
                                    //$scope.SubMenuItemIngredientsData.push();
                                }
...
//register form (ezhil)
.controller('registerForm', function($scope,$http,$location,$cordovaOauth) {  
    $("#sidebar").removeClass("toggled");
  $("#menu-trigger").removeClass("open");
  localStorage.setItem("splash", 1);
...
.controller('authentication', function($scope,$http,$location) {  
  ...
  .controller('latestOfferController', function($scope,$http,$location,alertmsg,$cordovaNetwork) {  
        var businessId=1;
        localStorage.setItem("splash", 1);
        $scope.cartCountGet= localStorage.getItem("cartCount");
         $("#cartCount").html($scope.cartCountGet);
...
.controller('myProfileDetail', function($scope,$http,$ionicPlatform, $rootScope,$cordovaCamera) {
    //image upload
    ...
    $scope.updateProfile = function(e) {
        ...
        //console.log($event.closest('.pmb-block'));
        ...
        //console.log(updateDetailData);
        //var myJsonString = JSON.stringify(updateDetailData);
        //console.log(myJsonString);
        $http.post(baseUrl+'branboxController/updateUserDetail', updateDetailData).
    ...
    $http.post(baseUrl+'branboxController/getUserDetail', data).
    then(function(response) {
        console.log(response.data);
        console.log(response.data.id);
        var db = window.openDatabase("Database", "1.0", "BranBox", 65535);
        db.transaction(function(tx){
            var j=0;
            tx.executeSql('CREATE TABLE IF NOT EXISTS enduser ( id TEXT, userName TEXT, businessId TEXT');
            tx.executeSql('INSERT OR REPLACE INTO enduser (id, userName, businessId) VALUES ("'+response.data.id+'","'+response.data.userName+'","'+response.data.businessId+'")',successID);
